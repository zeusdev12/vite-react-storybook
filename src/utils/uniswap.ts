...
const RPC_URL = `https://eth-mainnet.alchemyapi.io/v2/${process.env.REACT_APP_ALCHEMY_API_KEY}`;

const findUniswapBestRoute = async (
...
const parseRoute = (
  ...
  const bestRoute: V3Route | V2Route | MixedRoute = _.maxBy(routeAmounts, ({ amount }) => {
  ...
  bestRoute.protocol === Protocol.V3 || bestRoute.protocol === Protocol.MIXED
      ? bestRoute.pools.map((pool) => pool.fee)
